...
function CreateHabitModal({ isOpen, onClose, cluster, habits, onCreateHabit }) {
  ...
  const clusterTitle = cluster?.name ?? "Ungrouped";
  const targetClusterId = cluster?.id ?? null;
  ...
  const hasDuplicateName = habits.some(
    (h) =>
      ...
      (h.cluster_id ?? null) === targetClusterId,
  );
          ...
          <div>
            ...
            <h3 className="mt-1 text-lg tracking-tight text-slate-100">
              ...
              <span
                className="font-bold"
                style={{
                  color: cluster.color ? cluster.color : "#8E8E8E",
                }}
              >
                {clusterTitle}
              </span>{" "}
